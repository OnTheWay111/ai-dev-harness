...
const AFTER_CANARY = new Date(START + 13 * HOUR);

function validate(value) {
  return validateP12CanaryReport(value, { now: AFTER_CANARY });
}

function windows(count = 12) {
...
function report(overrides = {}) {
  return {
    ...
    conditions: {
      success: ["Goal Verification passed", "No P0/P1 for 12 continuous hours"],
...
test("accepts 12 contiguous healthy hours with Goal Verification and owned P2", () => {
  const result = validate(report());
  assert.equal(result.durationHours, 12);
  assert.equal(result.windowCount, 12);
  assert.equal(result.p2Count, 1);
});

test("rejects a shortened or future observation", () => {
  const shortened = report();
  shortened.observation.endedAt = new Date(START + 11 * HOUR).toISOString();
  shortened.observation.windows = windows(11);
  assert.throws(() => validate(shortened), /12/);
  assert.throws(
    () => validateP12CanaryReport(report(), { now: new Date(START + 11 * HOUR) }),
    /future/,
  );
});
...
test("rejects missing observation time and P0/P1 defects", () => {
  const gap = report();
  gap.observation.windows[5].startedAt = new Date(START + 6.5 * HOUR).toISOString();
